...
#define WEENSYOS_FIRST_PROCESS "friends"
#endif
...
bool show_memory;               // whether to show memory
...
void kernel(const char* command) {
    ...
    // set up process descriptors and run
    ...
    if (program_loader(command).present()) {
        process_setup(1, command);
        show_memory = strncmp(command, "allocator", 9) == 0;
        run(&ptable[1]);
    } else if (strcmp(command, "allocators") == 0) {
...
void process_setup(pid_t pid, const char* program_name) {
    ...
    loader.reset();
    uintptr_t first_addr = loader.va();
    for (; loader.present(); ++loader) {
    ...
    // allocate stack
    uintptr_t stack_addr = round_up(first_addr | 1, PROC_SIZE) - PAGESIZE;
    log_printf("%p\n", stack_addr);
